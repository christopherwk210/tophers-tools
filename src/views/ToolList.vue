  ...
  <ToolWindow title="Tools">
    <div class="list-group list-group-flush">
      <template v-for="(entry, entryIndex) of organizedTools">
        ...
        <router-link :key="entryIndex" v-if="entry.route" :to="entry.route" class="list-group-item list-group-item-action bg-secondary text-light user-select-none">
          <h5 class="mb-1">{{entry.title}}</h5>
          <p class="m-0" style="opacity: 0.4">{{entry.description}}</p>
        </router-link>
        <a :key="entryIndex" v-if="entry.url" :href="entry.url" class="list-group-item list-group-item-action bg-secondary text-light user-select-none">
          <h5 class="mb-1">{{entry.title}}</h5>
          <p class="m-0" style="opacity: 0.4">{{entry.description}}</p>
        </a>
      </template>
